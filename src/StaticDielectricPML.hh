...
StaticDielectricPML<X_ATTENUATION, Y_ATTENUATION, Z_ATTENUATION>::
...
template <bool X_ATTENUATION, bool Y_ATTENUATION, bool Z_ATTENUATION>
void StaticDielectricPML<X_ATTENUATION, Y_ATTENUATION, Z_ATTENUATION>::
onAddRunline(Runline & rl, int ii, int jj, int kk)
{
	...
	assert(mExtents.encloses(Vector3i(ii,jj,kk)));
	
	//LOG << "Runline addition: \n";
	//LOGMORE << mExtents << "\n";
    //LOGMORE << "Aux indices: \n";
	//	<< rl.getAuxIndex1() << " " << rl.getAuxIndex2() << " "
	//	<< rl.getAuxIndex3()
	//	<< " at " << ii << " " << jj << " " << kk << "\n";
...
template <bool X_ATTENUATION, bool Y_ATTENUATION, bool Z_ATTENUATION>
string StaticDielectricPML<X_ATTENUATION, Y_ATTENUATION, Z_ATTENUATION>::
getMaterialName() const
{
	ostringstream str;
	str << "Static Dielectric PML " << X_ATTENUATION << " "
...
stepEx(const Runline & rl, float dxinv, float dyinv, float dzinv, float dt)
{
    ...
    float dt_eps = dt / (Constants::eps0 * m_epsr);
    ...
    for (int ii = 0; ii < rlLength; ii++)
    {
		...
		*/
        ...
        // current timestep
		//assert(!isnan(*Ex));
        *Ex = *Ex + dHz*dt_eps*kyinv - dHy*dt_eps*kzinv;
		//assert(!isnan(*Ex));
        ...
        if (Y_ATTENUATION)
        {
            ...
            *Ex += dt_eps*(*psiY); // current timestep
            psiY++;
        }
        ...
        if (Z_ATTENUATION)
        {
            ...
            *Ex -= dt_eps*(*psiZ); // current timestep
            psiZ++;
        }
		//assert(!isnan(*Ex));
...
stepEy(const Runline & rl, float dxinv, float dyinv, float dzinv, float dt)
{
    ...
    for (int ii = 0; ii < rlLength; ii++)
    {
        ...
        *Ey = *Ey + dHx*dt_eps*kzinv - dHz*dt_eps*kxinv;
        ...
        if (Z_ATTENUATION)
        {
            ...
            *Ey += dt_eps*(*psiZ); // current timestep
            psiZ++;
        }
...
stepEz(const Runline & rl, float dxinv, float dyinv, float dzinv, float dt)
{
    ...
    for (int ii = 0; ii < rlLength; ii++)
    {
        ...
        *Ez = *Ez + dHy*dt_eps*kxinv - dHx*dt_eps*kyinv;
        ...
        if (Y_ATTENUATION)
        {
            ...
            *Ez -= dt_eps*(*psiY); // current timestep            
            psiY++;
        }
...
stepEx_split(const Runline & rl, float dxinv, float dyinv, float dzinv, float dt)
{
	...
	// All that depends on curl H goes first.
    if (Y_ATTENUATION)
    {   
        psiY = &mAccumEk[1][rl.getIndex()];
        ay = mAEk[1][rl.getAuxIndex2()];
...
stepEy_split(const Runline & rl, float dxinv, float dyinv, float dzinv, float dt)
{
	...
	// Second part: no curl H dependence
...
template <bool X_ATTENUATION, bool Y_ATTENUATION, bool Z_ATTENUATION>
void StaticDielectricPML<X_ATTENUATION, Y_ATTENUATION, Z_ATTENUATION>::
stepEz_unrolled(const Runline & rl, float dxinv, float dyinv, float dzinv, float dt)
{
    float* Ez = rl.field0();
    const float* HxN = rl.field1Neg();
    const float* HxP = rl.field1Pos();
